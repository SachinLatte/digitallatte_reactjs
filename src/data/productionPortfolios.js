export const productionPortfoliosData = {
  "rentio-tea-photography-and-videoshoot": {
    slug: "rentio-tea-photography-and-videoshoot",
    client: "Rentio Tea",
    bannerTitle: {
      highlight: "Rentio Tea",
      regular: "Portfolio"
    },
    metaTitle: "FMCG Photography | Rentio Tea | Photo and Video Shoot",
    metaDescription:
      "Rentio Tea is a name synonymous with purity, tradition, and an uncompromising love for tea. Digital Latte is proud to present this beautifully crafted video that pays homage to the brand's legacy.",
    projectSummary: [
      "Rentio Tea is a name synonymous with purity, tradition, and an uncompromising love for tea. Digital Latte is proud to present this beautifully crafted video that pays homage to the brand's legacy.",
      "From the misty tea gardens to the warmth of your favourite cup, we've captured the journey of flavour and tradition. Here’s to a collaboration that's steeped in excellence, just like every brew of Rentio Tea."
    ],
    subheadQuote:
      "Boost your social media & digital marketing strategies with beautiful photo & video shoot, and engaging experiences.",
    showTabs: true,
    items: [
      {
        id: 1,
        type: "image",
        size: "size1",
        src: "/img/photography/rentio-gallery/img1.jpg",
        title: "Rentio Tea Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 2,
        type: "image",
        size: "size2",
        src: "/img/photography/rentio-gallery/img2.jpg",
        title: "Rentio Tea Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 3,
        type: "image",
        size: "size1",
        src: "/img/photography/rentio-gallery/img3.jpg",
        title: "Rentio Tea Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 4,
        type: "image",
        size: "size2",
        src: "/img/photography/rentio-gallery/img4.jpg",
        title: "Rentio Tea Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 5,
        type: "image",
        size: "size2",
        src: "/img/photography/rentio-gallery/img5.jpg",
        title: "Rentio Tea Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 6,
        type: "image",
        size: "size2",
        src: "/img/photography/rentio-gallery/img6.jpg",
        title: "Rentio Tea Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 7,
        type: "image",
        size: "size2",
        src: "/img/photography/rentio-gallery/img7.jpg",
        title: "Rentio Tea Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 8,
        type: "image",
        size: "size2",
        src: "/img/photography/rentio-gallery/img8.jpg",
        title: "Rentio Tea Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 9,
        type: "image",
        size: "size1",
        src: "/img/photography/rentio-gallery/img9.jpg",
        title: "Rentio Tea Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 10,
        type: "image",
        size: "size1",
        src: "/img/photography/rentio-gallery/img10.jpg",
        title: "Rentio Tea Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 11,
        type: "video",
        size: "size1",
        src: "/img/photography/rentio-gallery/img11.jpg",
        youtubeId: "oTpmY9kcvVA",
        title: "Rentio Tea Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 12,
        type: "video",
        size: "size2",
        src: "/img/photography/rentio-gallery/img12.jpg",
        youtubeId: "Ri9PhS47L3Q",
        title: "Rentio Tea Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 13,
        type: "video",
        size: "size1",
        src: "/img/photography/rentio-gallery/img13.jpg",
        youtubeId: "RR1cYRThuFg",
        title: "Rentio Tea Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 14,
        type: "video",
        size: "size2",
        src: "/img/photography/rentio-gallery/img14.jpg",
        youtubeId: "ujrEcNrY5Ws",
        title: "Rentio Tea Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 15,
        type: "video",
        size: "size2",
        src: "/img/photography/rentio-gallery/img15.jpg",
        youtubeId: "h2ooxhkWs18",
        title: "Rentio Tea Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 16,
        type: "video",
        size: "size2",
        src: "/img/photography/rentio-gallery/img16.jpg",
        youtubeId: "f_Bvpc11wkQ",
        title: "Rentio Tea Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 17,
        type: "video",
        size: "size2",
        src: "/img/photography/rentio-gallery/img17.jpg",
        youtubeId: "Z5I1VDUq2Tw",
        title: "Rentio Tea Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 18,
        type: "video",
        size: "size2",
        src: "/img/photography/rentio-gallery/img18.jpg",
        youtubeId: "0FQ6pcY-_1I",
        title: "Rentio Tea Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 19,
        type: "video",
        size: "size1",
        src: "/img/photography/rentio-gallery/img19.jpg",
        youtubeId: "ODPaOUT7BVk",
        title: "Rentio Tea Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 20,
        type: "video",
        size: "size1",
        src: "/img/photography/rentio-gallery/img20.jpg",
        youtubeId: "bywo4DW1yww",
        title: "Rentio Tea Video Shoot",
        subtitle: "Play Video"
      }
    ],
    prevPortfolio: {
      clientName: "Patna Pirates",
      title: "Patna Pirates",
      slug: "/our-expertise/digital-marketing-services/patna-pirates-photography-and-videoshoot"
    },
    nextPortfolio: {
      clientName: "J. Hampstead",
      title: "J. Hampstead",
      slug: "/our-expertise/digital-marketing-services/j-hampstead-photography-and-videoshoot"
    }
  },

  "patna-pirates-photography-and-videoshoot": {
    slug: "patna-pirates-photography-and-videoshoot",
    client: "Patna Pirates",
    bannerTitle: {
      highlight: "Patna Pirates",
      regular: "Portfolio"
    },
    metaTitle: "Patna Pirates Photography & Videoshoot | Digital Latte",
    metaDescription:
      "Patna Pirates is one of the most successful kabaddi franchise in Pro Kabaddi League. We conceptualised thematic photoshoot & videography to bring out team's pride and the personality of the athletes.",
    projectSummary: [
      "Patna Pirates is one of the most successful kabaddi franchise in Pro Kabaddi League. We conceptualised thematic photoshoot & videography to bring out team's pride and the personality of the athletes."
    ],
    subheadQuote:
      "Boost your social media & digital marketing strategies with beautiful photo & video shoot, and engaging experiences.",
    showTabs: true,
    items: [
      {
        id: 1,
        type: "image",
        size: "size1",
        src: "/img/photography/patna-pirates/img1.jpg",
        title: "Patna Pirates Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 2,
        type: "image",
        size: "size2",
        src: "/img/photography/patna-pirates/img2.jpg",
        title: "Patna Pirates Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 3,
        type: "image",
        size: "size1",
        src: "/img/photography/patna-pirates/img3.jpg",
        title: "Patna Pirates Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 4,
        type: "image",
        size: "size2",
        src: "/img/photography/patna-pirates/img4.jpg",
        title: "Patna Pirates Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 5,
        type: "image",
        size: "size2",
        src: "/img/photography/patna-pirates/img5.jpg",
        title: "Patna Pirates Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 6,
        type: "image",
        size: "size2",
        src: "/img/photography/patna-pirates/img6.jpg",
        title: "Patna Pirates Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 7,
        type: "image",
        size: "size2",
        src: "/img/photography/patna-pirates/img7.jpg",
        title: "Patna Pirates Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 8,
        type: "image",
        size: "size2",
        src: "/img/photography/patna-pirates/img8.jpg",
        title: "Patna Pirates Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 9,
        type: "image",
        size: "size1",
        src: "/img/photography/patna-pirates/img9.jpg",
        title: "Patna Pirates Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 10,
        type: "image",
        size: "size1",
        src: "/img/photography/patna-pirates/img10.jpg",
        title: "Patna Pirates Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 11,
        type: "video",
        size: "size1",
        src: "/img/photography/patna-pirates/img11.jpg",
        youtubeId: "YZgI_KNDBbk",
        title: "Patna Pirates Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 12,
        type: "video",
        size: "size2",
        src: "/img/photography/patna-pirates/img12.jpg",
        youtubeId: "BYigvQWqxU0",
        title: "Patna Pirates Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 13,
        type: "video",
        size: "size1",
        src: "/img/photography/patna-pirates/img13.jpg",
        youtubeId: "LS4kyJACjus",
        title: "Patna Pirates Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 14,
        type: "video",
        size: "size2",
        src: "/img/photography/patna-pirates/img14.jpg",
        youtubeId: "oo6R7FYaJ7g",
        title: "Patna Pirates Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 15,
        type: "video",
        size: "size2",
        src: "/img/photography/patna-pirates/img15.jpg",
        youtubeId: "qLMi4sdpvbI",
        title: "Patna Pirates Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 16,
        type: "video",
        size: "size2",
        src: "/img/photography/patna-pirates/img16.jpg",
        youtubeId: "V1jxnhQmUyc",
        title: "Patna Pirates Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 17,
        type: "video",
        size: "size2",
        src: "/img/photography/patna-pirates/img17.jpg",
        youtubeId: "I2AOEtLiERk",
        title: "Patna Pirates Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 18,
        type: "video",
        size: "size2",
        src: "/img/photography/patna-pirates/img18.jpg",
        youtubeId: "WfKiGxjAJrY",
        title: "Patna Pirates Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 19,
        type: "video",
        size: "size1",
        src: "/img/photography/patna-pirates/img19.jpg",
        youtubeId: "l5gJVuoLclw",
        title: "Patna Pirates Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 20,
        type: "video",
        size: "size1",
        src: "/img/photography/patna-pirates/img20.jpg",
        youtubeId: "dQhOUeFn_I0",
        title: "Patna Pirates Video Shoot",
        subtitle: "Play Video"
      }
    ],
    prevPortfolio: {
      clientName: "Goldiee Masale",
      title: "Goldiee Masale",
      slug: "/our-expertise/digital-marketing-services/goldiee-masale-photography-and-videoshoot"
    },
    nextPortfolio: {
      clientName: "J. Hampstead",
      title: "J. Hampstead",
      slug: "/our-expertise/digital-marketing-services/j-hampstead-photography-and-videoshoot"
    }
  },

  "mmfgroup-corporate-warehouse-videoshoot": {
    slug: "mmfgroup-corporate-warehouse-videoshoot",
    client: "Manoj Multifoods",
    bannerTitle: {
      highlight: "MMF Group",
      regular: "Portfolio"
    },
    metaTitle: "MMF Group Photography & Video Production | Digital Latte",
    metaDescription:
      "We executed a multi-location photography and video production project for Manoj Multifoods (MMF Group), a Super Stockist and Integrated Logistics Partner for FMCG and Food Brands.",
    projectSummary: [
      "We executed a multi-location photography and video production project for Manoj Multifoods (MMF Group), a Super Stockist and Integrated Logistics Partner for FMCG and Food Brands.",
      "The shoot spanned across their warehouse facilities & corporate offices encompassing aerial drone videography, cinematography, interview shoots, and the creation of corporate films, all aimed at capturing the brand's operational excellence and organisational culture.",
      "Through strategic direction, precise coordination, and creative storytelling, the production showcased MMF Group's modern infrastructure, efficient processes, and dynamic workforce."
    ],
    subheadQuote:
      "Boost your social media & digital marketing strategies with beautiful photo & video shoot, and engaging experiences.",
    showTabs: false,
    items: [
      {
        id: 1,
        type: "image",
        size: "size1",
        src: "/img/photography/mmf-group/img1.webp",
        title: "MMF Group Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 2,
        type: "image",
        size: "size2",
        src: "/img/photography/mmf-group/img2.webp",
        title: "MMF Group Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 3,
        type: "image",
        size: "size1",
        src: "/img/photography/mmf-group/img3.webp",
        title: "MMF Group Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 4,
        type: "image",
        size: "size2",
        src: "/img/photography/mmf-group/img4.webp",
        title: "MMF Group Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 5,
        type: "image",
        size: "size2",
        src: "/img/photography/mmf-group/img5.webp",
        title: "MMF Group Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 6,
        type: "image",
        size: "size2",
        src: "/img/photography/mmf-group/img6.webp",
        title: "MMF Group Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 7,
        type: "image",
        size: "size2",
        src: "/img/photography/mmf-group/img7.webp",
        title: "MMF Group Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 8,
        type: "image",
        size: "size2",
        src: "/img/photography/mmf-group/img8.webp",
        title: "MMF Group Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 9,
        type: "image",
        size: "size1",
        src: "/img/photography/mmf-group/img9.webp",
        title: "MMF Group Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 10,
        type: "image",
        size: "size1",
        src: "/img/photography/mmf-group/img10.webp",
        title: "MMF Group Photo Shoot",
        subtitle: "View Large"
      }
    ],
    prevPortfolio: {
      clientName: "Goldiee Masale",
      title: "Goldiee Masale",
      slug: "/our-expertise/digital-marketing-services/goldiee-masale-photography-and-videoshoot"
    },
    nextPortfolio: {
      clientName: "J. Hampstead",
      title: "J. Hampstead",
      slug: "/our-expertise/digital-marketing-services/j-hampstead-photography-and-videoshoot"
    }
  },

  "goldiee-masale-photography-and-videoshoot": {
    slug: "goldiee-masale-photography-and-videoshoot",
    client: "Goldiee Masale",
    bannerTitle: {
      highlight: "Goldiee Masale",
      regular: "Portfolio"
    },
    metaTitle: "Goldiee Masale Photography & Videoshoot | Digital Latte",
    metaDescription:
      "A pinch of tradition, a dash of creativity, and a whole lot of passion, this is how Digital Latte captured the magic of Goldiee Masale.",
    projectSummary: [
      "A pinch of tradition, a dash of creativity, and a whole lot of passion, this is how Digital Latte captured the magic of Goldiee Masale. As storytellers, we focused on the emotional connection that spices create, turning every meal into a celebration of culture and taste.",
      "Goldiee Masale is more than just spices it's an experience and we're thrilled to be part of its journey!"
    ],
    subheadQuote:
      "Boost your social media & digital marketing strategies with beautiful photo & video shoot, and engaging experiences.",
    showTabs: true,
    items: [
      {
        id: 1,
        type: "image",
        size: "size1",
        src: "/img/photography/goldiee-masale-gallery/img1.jpg",
        title: "Goldiee Masale Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 2,
        type: "image",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img2.jpg",
        title: "Goldiee Masale Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 3,
        type: "image",
        size: "size1",
        src: "/img/photography/goldiee-masale-gallery/img3.jpg",
        title: "Goldiee Masale Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 4,
        type: "image",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img4.jpg",
        title: "Goldiee Masale Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 5,
        type: "image",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img5.jpg",
        title: "Goldiee Masale Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 6,
        type: "image",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img6.jpg",
        title: "Goldiee Masale Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 7,
        type: "image",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img7.jpg",
        title: "Goldiee Masale Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 8,
        type: "image",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img8.jpg",
        title: "Goldiee Masale Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 9,
        type: "image",
        size: "size1",
        src: "/img/photography/goldiee-masale-gallery/img9.jpg",
        title: "Goldiee Masale Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 10,
        type: "image",
        size: "size1",
        src: "/img/photography/goldiee-masale-gallery/img10.jpg",
        title: "Goldiee Masale Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 11,
        type: "video",
        size: "size1",
        src: "/img/photography/goldiee-masale-gallery/img11.jpg",
        youtubeId: "xdVN7uKv0FA",
        title: "Goldiee Masale Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 12,
        type: "video",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img12.jpg",
        youtubeId: "0JsMH0cnfDU",
        title: "Goldiee Masale Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 13,
        type: "video",
        size: "size1",
        src: "/img/photography/goldiee-masale-gallery/img13.jpg",
        youtubeId: "cQxdadiJV5k",
        title: "Goldiee Masale Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 14,
        type: "video",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img14.jpg",
        youtubeId: "yPbwYYIsiOk",
        title: "Goldiee Masale Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 15,
        type: "video",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img15.jpg",
        youtubeId: "Zqx46tznfaU",
        title: "Goldiee Masale Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 16,
        type: "video",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img16.jpg",
        youtubeId: "2XccSFt8kaQ",
        title: "Goldiee Masale Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 17,
        type: "video",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img17.jpg",
        youtubeId: "v4iaiVLJ-0Q",
        title: "Goldiee Masale Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 18,
        type: "video",
        size: "size2",
        src: "/img/photography/goldiee-masale-gallery/img18.jpg",
        youtubeId: "LPNh-3FnuZc",
        title: "Goldiee Masale Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 19,
        type: "video",
        size: "size1",
        src: "/img/photography/goldiee-masale-gallery/img19.jpg",
        youtubeId: "1AhE9sT9PNI",
        title: "Goldiee Masale Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 20,
        type: "video",
        size: "size1",
        src: "/img/photography/goldiee-masale-gallery/img20.jpg",
        youtubeId: "oA09K3jLtbM",
        title: "Goldiee Masale Video Shoot",
        subtitle: "Play Video"
      }
    ],
    prevPortfolio: {
      clientName: "Rentio Tea",
      title: "Rentio Tea",
      slug: "/our-expertise/digital-marketing-services/rentio-tea-photography-and-videoshoot"
    },
    nextPortfolio: {
      clientName: "J. Hampstead",
      title: "J. Hampstead",
      slug: "/our-expertise/digital-marketing-services/j-hampstead-photography-and-videoshoot"
    }
  },

  "j-hampstead-photography-and-videoshoot": {
    slug: "j-hampstead-photography-and-videoshoot",
    client: "J. Hampstead",
    bannerTitle: {
      highlight: "J. Hampstead",
      regular: "Portfolio"
    },
    metaTitle: "J. Hampstead Photography & Videoshoot | Digital Latte",
    metaDescription:
      "J. Hampstead stands for style, elegance, and craftsmanship. Digital Latte had the privilege of creating this captivating visual story that celebrates the brand's iconic legacy.",
    projectSummary: [
      "J. Hampstead stands for style, elegance, and craftsmanship. Digital Latte had the privilege of creating this captivating visual story that celebrates the brand's iconic legacy. From rich textures to contemporary designs, every detail was brought to life with an eye for perfection.",
      "Let us take you on this journey of style, innovation, and artistry. Here's a glimpse of what makes J. Hampstead an icon in the world of fashion because luxury deserves to be showcased with perfection."
    ],
    subheadQuote:
      "Boost your social media & digital marketing strategies with beautiful photo & video shoot, and engaging experiences.",
    showTabs: true,
    items: [
      {
        id: 1,
        type: "image",
        size: "size1",
        src: "/img/photography/j-hampstead-gallery/img1.jpg",
        title: "J. Hampstead Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 2,
        type: "image",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img2.jpg",
        title: "J. Hampstead Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 3,
        type: "image",
        size: "size1",
        src: "/img/photography/j-hampstead-gallery/img3.jpg",
        title: "J. Hampstead Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 4,
        type: "image",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img4.jpg",
        title: "J. Hampstead Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 5,
        type: "image",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img5.jpg",
        title: "J. Hampstead Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 6,
        type: "image",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img6.jpg",
        title: "J. Hampstead Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 7,
        type: "image",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img7.jpg",
        title: "J. Hampstead Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 8,
        type: "image",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img8.jpg",
        title: "J. Hampstead Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 9,
        type: "image",
        size: "size1",
        src: "/img/photography/j-hampstead-gallery/img9.jpg",
        title: "J. Hampstead Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 10,
        type: "image",
        size: "size1",
        src: "/img/photography/j-hampstead-gallery/img10.jpg",
        title: "J. Hampstead Photo Shoot",
        subtitle: "View Large"
      },
      {
        id: 11,
        type: "video",
        size: "size1",
        src: "/img/photography/j-hampstead-gallery/img11.jpg",
        youtubeId: "8ohgUtKFyXU",
        title: "J. Hampstead Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 12,
        type: "video",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img12.jpg",
        youtubeId: "kk_m-RHfV8U",
        title: "J. Hampstead Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 13,
        type: "video",
        size: "size1",
        src: "/img/photography/j-hampstead-gallery/img13.jpg",
        youtubeId: "yqaZ33OowS8",
        title: "J. Hampstead Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 14,
        type: "video",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img14.jpg",
        youtubeId: "viiqE2iKbmM",
        title: "J. Hampstead Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 15,
        type: "video",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img15.jpg",
        youtubeId: "G9ccIegc5qk",
        title: "J. Hampstead Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 16,
        type: "video",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img16.jpg",
        youtubeId: "LWRshqgJPe0",
        title: "J. Hampstead Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 17,
        type: "video",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img17.jpg",
        youtubeId: "O5HoS06tLVc",
        title: "J. Hampstead Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 18,
        type: "video",
        size: "size2",
        src: "/img/photography/j-hampstead-gallery/img18.jpg",
        youtubeId: "mtDe0G16LmA",
        title: "J. Hampstead Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 19,
        type: "video",
        size: "size1",
        src: "/img/photography/j-hampstead-gallery/img19.jpg",
        youtubeId: "1Dn8hw7kVUA",
        title: "J. Hampstead Video Shoot",
        subtitle: "Play Video"
      },
      {
        id: 20,
        type: "video",
        size: "size1",
        src: "/img/photography/j-hampstead-gallery/img20.jpg",
        youtubeId: "Q53fQDFY8lQ",
        title: "J. Hampstead Video Shoot",
        subtitle: "Play Video"
      }
    ],
    prevPortfolio: {
      clientName: "Rentio Tea",
      title: "Rentio Tea",
      slug: "/our-expertise/digital-marketing-services/rentio-tea-photography-and-videoshoot"
    },
    nextPortfolio: {
      clientName: "Goldiee Masale",
      title: "Goldiee Masale",
      slug: "/our-expertise/digital-marketing-services/goldiee-masale-photography-and-videoshoot"
    }
  }
};

export default productionPortfoliosData;
